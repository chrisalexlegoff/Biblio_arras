...
class UtilisateurController
{
    ...
    public function supprimerUtilisateurByAdmin($iDutilisateur) {
        ...
        if ($resultat) {
            ...
            $_SESSION['erreurs'][] = [['update-utilisateur' => 'Erreur de suppression']];
            header('location: ' . SITE_URL . 'gestion-membres');
            exit;
        }
    }
    ...
    public function modificationProfil($idUtilisateur) {
        ...
        $erreurs = $this->validationDonnees->valider([
            ...
            'password' => (!empty($_POST['password'])) ? ['required', 'match:/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*\W)(?!.* ).{12,}$/'] : [],
        ], $_POST);
        if ($_POST['password_check'] !== $_POST['password']) {
            $_SESSION['erreurs'][] = [['Les 2 mots de passe ne correspondent pas!']];
        ...
        $resultat = $this->utilisateurRepository->modifierUtilisateurBdd($idUtilisateur);
        if ($resultat) {
            $_SESSION['alert'] = [
                "type" => "success",
    ...
    public function connexionValidation()
    {
        ...
        if ($utilisateur) {
            if (password_verify($_POST['password'], $utilisateur->getPassword())) {
                ...
                $_SESSION['erreurs'][] = [['email' => 'Email ou mot de passe incorrect']];
                header('location: ' . SITE_URL . 'connexion'); // on redirige vers la page d'ajout
                exit;
            }
        } else {
            $_SESSION['erreurs'][] = [['email' => 'Email ou mot de passe incorrect']];
